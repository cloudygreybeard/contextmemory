...
# Test 4: Linting
echo "4️⃣  Testing linting..."
if make lint > /dev/null 2>&1; then
    echo "✅ No linting errors"
else
    echo "⚠️  Linting issues found (non-critical for functional release)"
fi

# Test 5: CLI Functional Tests
echo "5️⃣  Running CLI functional tests..."
if bash scripts/test-cli.sh; then
    echo "✅ CLI functional tests passed"
else
    echo "❌ CLI functional tests failed"
    exit 1
fi

# Test 6: Extension Packaging
echo "6️⃣  Testing extension packaging..."
...
if [[ "$CLI_VERSION" == "$UI_VERSION" && "$UI_VERSION" == "$ROOT_VERSION" ]]; then
    ...
    echo "❌ Version mismatch: CLI=$CLI_VERSION, UI=$UI_VERSION, Root=$ROOT_VERSION"
    exit 1
fi

# Test 8: Multi-Architecture Support (if available)
echo "8️⃣  Testing cross-compilation..."
PLATFORMS_TESTED=0
...
if [[ "$PLATFORMS_TESTED" -ge 1 ]]; then
    ...
    echo "⚠️  Cross-compilation limited (non-critical for functional release)"
fi
